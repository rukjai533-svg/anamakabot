...
FLAT_FEE       = 0      # not used anymore — fee is % based
INR_FEE_PCT    = 5
CRYPTO_FEE_PCT = 3
...
def parse_form(text):
    ...
    REQUIRED: buyer username, seller username
    OPTIONAL: timeframe, amount, network/payment, terms/condition
    Handles: numbered lists, dash separator, colon separator,
             missing fields, typos, any order
    """
    result = {}
    ...
    for line in lines:
        ...
        # Skip notes section
        if low.startswith("🔒") or low.startswith("•") or low.startswith("note"):
            continue

        # Remove leading number like "1." "2." etc
        line = re.sub(r"^\d+[\.\)]\s*", "", line).strip()
        low  = line.lower()

        # Split on : or - (take first separator)
        if ":" in line:
        ...
        uname_match = re.search(r"@?([A-Za-z0-9_]{4,32})", val)
        clean_val   = val.lstrip("@").split()[0].strip(".,") if val else ""

        if "buyer" in key:
            if val:
                result["buyer"] = clean_val

        elif "seller" in key:
            if val:
                result["seller"] = clean_val

        elif any(w in key for w in ["time","complete","completion","timeframe","duration"]):
            if val:
                result["timeframe"] = val

        elif any(w in key for w in ["amount","deal amount","price","worth"]):
            if val:
        ...
        elif any(w in key for w in ["network","payment","mode","method","pay"]):
            if val:
                result["network"] = val

        elif any(w in key for w in ["condition","escrow condition","terms","term"]):
            if val:
                result["condition"] = val

    # Terms fallback — look for "Terms and Condition [...]" line
    for line in lines:
        if "terms" in line.lower() and "[" in line:
            t = re.search(r"\[(.+?)\]", line)
            if t:
                result["terms"] = t.group(1).strip()
            break

    # Use condition as terms if terms missing
    if "terms" not in result and "condition" in result:
        result["terms"] = result["condition"]
...
def cmd_deal(message):
    ...
    if not message.reply_to_message:
        bot.reply_to(message, "ℹ️ Reply on the filled form and type /deal.")
        return
    ...
    # ONLY buyer and seller are required
    missing = [f for f in ["buyer","seller"] if not form.get(f)]
    if missing:
        bot.reply_to(message,
            "❌ Buyer and Seller username must be in the form.\n"
    ...
    terms     = form.get("terms", form.get("condition", ""))

    # Auto detect INR or Crypto and apply correct fee %
    ctype, csym, fee_pct = detect_currency(
        form.get("amount_raw", ""), network)
    ...
    # ── Deal card — skip blank optional fields ──
    lines = [
    ...
    if terms:
        lines.append({"text": f"🔐 Escrow Condition: {terms}"})
    if timeframe:
        lines.append({"text": f"⏱ Timeframe: {timeframe}"})
    if amount:
        lines.append({"text": f"💵 Deal Amount: {csym}{fmt(amount)}", "emojis": [{"char":"💵","id":E_DOLLAR}]})
    if network:
        lines.append({"text": f"🗂 Mode of Payment: {network}"})
    lines += [
        {"text": f"🛡 Escrow Fee: {fee_pct}% ({csym}{fmt(fee)})", "emojis": [{"char":"🛡","id":E_FEES}]},
        {"text": f"💰 Total Payable: {csym}{fmt(total)}"},
        {"text": ""},
    ...
    card_text = f"🔒 DEAL ID: #{deal_id}\n\n👤 Buyer: @{form['buyer']}\n👤 Seller: @{form['seller']}\n"
    if terms:     card_text += f"🔐 Escrow Condition: {terms}\n"
    if timeframe: card_text += f"⏱ Timeframe: {timeframe}\n"
    if amount:    card_text += f"💵 Deal Amount: {csym}{fmt(amount)}\n"
    if network:   card_text += f"🗂 Mode of Payment: {network}\n"
    card_text += f"🛡 Escrow Fee: {fee_pct}% ({csym}{fmt(fee)})\n💰 Total Payable: {csym}{fmt(total)}\n\n🔑 Escrower: @{escrow_uname}\n\n📋 Please review and confirm the deal.\nAuto-cancels in 15 minutes if not confirmed."
...
# ──────────────────────────────────────────────────
#  /complete DEAL_ID — Complete + Vouch
# ──────────────────────────────────────────────────
...
def cmd_complete(message):
    ...
    vouch_lines = [
        ...
        {"text": f"💵 Released Amount: {csym}{fmt(amount)}", "emojis": [{"char":"💵","id":E_DOLLAR}]},
    ...
    try:
        send_vouch_to_channel(deal_id, deal, amount, csym, escrow_name, escrow_id, date_str)
    except Exception as ex:
        bot.send_message(message.chat.id, f"⚠️ Vouch channel error: {ex}")


def send_vouch_to_channel(deal_id, deal, amount, csym, escrow_name, escrow_id, date_str):
    vouch_lines = [
        {"text": "✅ Deal Completed", "emojis": [{"char":"✅","id":E_TICK}]},
        {"text": ""},
        {"text": f"🔒 Trade ID: #{deal_id}", "emojis": [{"char":"🔒","id":E_LOCK}]},
...
def cmd_help(message):
    bot.send_message(message.chat.id,
        ...
        "/received       → Mark payment received - auto detect (Admin)\n"
        "/complete       → Complete deal + vouch - auto detect (Admin)\n"
        ...
        "Currency auto-detected from form!"
    )
